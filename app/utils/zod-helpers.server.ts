...
function getParamsInternal<T>(
  params: FormData | Record<string, string | undefined> | URLSearchParams,
  schema: any
):
  | { data: T; errors: undefined; success: true }
  | { data: undefined; errors: { [key: string]: string }; success: false } {
  // @ts-ignore
  let o: any = {};
  ...
  if (result.success) {
    return { data: result.data as T, errors: undefined, success: true };
  } else {
    let errors: any = {};
    ...
    for (let issue of result.error.issues) {
      // const { code, expected, message, path, received } = issue;
    ...
    return { data: undefined, errors, success: false };
  }
}
